...
public class SendStartGameCommand extends Command {
    ...
    public SendStartGameCommand(String label, Proxy proxy) {
        super(label);
        this.proxy = proxy;
        init();
    }
    ...
    private void init() {
        ...
        CommandArgument manualSingle = new CommandArgument("-manual", 2, new CommandArgument[]{
                new CommandArgumentValue(3, ctx -> singleManual(ctx))
        }, null);
    ...
    private void printHelp() {
        System.out.println("Usage for sendstartgame:");
        System.out.println("  sendstartgame -help");
        System.out.println("  sendstartgame -broadcast -random [-action <Generic|Add|Update|Remove|Replace>]");
        System.out.println("  sendstartgame -broadcast -manual -playerid <id> [-action <Generic|Add|Update|Remove|Replace>]");
        System.out.println("  sendstartgame -singleconnection <targetPlayerId> -random [-action <Generic|Add|Update|Remove|Replace>]");
        System.out.println("  sendstartgame -singleconnection <targetPlayerId> -manual -playerid <id> [-action <Generic|Add|Update|Remove|Replace>]");
        System.out.println();
    ...
    private void broadcastRandom(CommandContext ctx) {
        ...
        System.out.println("Sent StartGame packet to all clients: " + packet);
    }
    ...
    private void broadcastManual(CommandContext ctx) {
        ...
        if (playerId == null) {
            System.out.println("Usage: sendstartgame -broadcast -manual -playerid <id> [-action <action>]");
            return;
        }
        StartGame packet = new StartGame(playerId, action);
        proxy.broadcast(packet);
        System.out.println("Sent StartGame packet to all clients: " + packet);
    }
    ...
    private void singleRandom(CommandContext ctx) {
        String[] args = ctx.args();
        if (args.length < 2) {
            System.out.println("Usage: sendstartgame -singleconnection <playerId> -random");
            return;
        }
        ...
        System.out.println("Sent StartGame packet to " + targetPlayer + ": " + packet);
    }
    ...
    private void singleManual(CommandContext ctx) {
        ...
        if (playerId == null) {
            System.out.println("Usage: sendstartgame -singleconnection <targetPlayerId> -manual -playerid <id>");
            return;
        }
        StartGame packet = new StartGame(playerId, PacketAction.Add);
        proxy.send(targetPlayer, packet);
        System.out.println("Sent StartGame packet to " + targetPlayer + ": " + packet);
    }
} 