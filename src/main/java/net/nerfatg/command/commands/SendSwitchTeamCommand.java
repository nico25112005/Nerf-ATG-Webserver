...
public class SendSwitchTeamCommand extends Command {
    ...
    public SendSwitchTeamCommand(String label, Proxy proxy) {
        super(label);
        this.proxy = proxy;
        init();
    }
    ...
    private void printHelp() {
        System.out.println("Usage for sendswitchteam:");
        System.out.println("  sendswitchteam -help");
        System.out.println("  sendswitchteam -broadcast -random [-action <Generic|Add|Update|Remove|Replace>]");
        System.out.println("  sendswitchteam -broadcast -manual -playerid <id> [-action <Generic|Add|Update|Remove|Replace>]");
        System.out.println("  sendswitchteam -singleconnection <targetPlayerId> -random [-action <Generic|Add|Update|Remove|Replace>]");
        System.out.println("  sendswitchteam -singleconnection <targetPlayerId> -manual -playerid <id> [-action <Generic|Add|Update|Remove|Replace>]");
        System.out.println();
    ...
    private void broadcastManual(CommandContext ctx) {
        ...
        if (playerId == null) {
            System.out.println("Usage: sendswitchteam -broadcast -manual -playerid <id> [-action <action>]");
            return;
        }
        SwitchTeam packet = new SwitchTeam(playerId, action);
        proxy.broadcast(packet);
        System.out.println("Sent SwitchTeam packet to all clients: " + packet);
    }
    ...
    private void singleRandom(CommandContext ctx) {
        String[] args = ctx.args();
        if (args.length < 2) {
            System.out.println("Usage: sendswitchteam -singleconnection <playerId> -random");
            return;
        }
    ...
    private void singleManual(CommandContext ctx) {
        ...
        if (playerId == null) {
            System.out.println("Usage: sendswitchteam -singleconnection <targetPlayerId> -manual -playerid <id>");
            return;
        }
        SwitchTeam packet = new SwitchTeam(playerId, PacketAction.Add);
        proxy.send(targetPlayer, packet);
        System.out.println("Sent SwitchTeam packet to " + targetPlayer + ": " + packet);
    }
} 